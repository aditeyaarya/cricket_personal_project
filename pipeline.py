"""
T20 Cricket Win Probability Pipeline  —  v8 (Wave 4)
=====================================================
Changes vs v7
--------------
  WAVE 4 — Rolling player form + advanced game state features.

  NEW FEATURES (computed by wave4.py):
    batsman_sr_2yr           — current batsman's 2yr rolling strike rate
    batsman_boundary_pct_2yr — boundary hitting frequency
    batsman_dot_pct_2yr      — dot ball tendency
    bowler_economy_2yr       — current bowler's 2yr rolling economy
    bowler_dot_pct_2yr       — bowler's dot ball frequency
    momentum_index           — batting acceleration/deceleration
    over_par                 — runs ahead/behind venue median at this over
                               (innings 1 only — KEY first innings signal)

  INNINGS 1 STRATEGY:
    The over_par feature directly tells Model T whether the current score
    is tracking high or low relative to history at this venue.  Combined
    with batsman_sr_2yr and bowler_economy_2yr, Model T now has per-delivery
    player-quality signals to resolve early-innings uncertainty.

    All 7 new features added to INN1_FEATURES (Model T).
    batsman_sr_2yr, bowler_economy_2yr, momentum_index added to WIN_FEATURES.

  DROPPED from WIN_FEATURES (zero importance in v7):
    bowling_strength_remaining, balls_remaining_before, wickets_last_12

  ADDED to WIN_FEATURES:
    pressure_index (compound game state for innings 2)
"""
...
if WAVE4_BALLS.exists():
    print(f"\nLoading Wave 4 enriched balls from {WAVE4_BALLS}")
    balls = pd.read_csv(WAVE4_BALLS)
    missing = [c for c in WAVE2_COLS + WAVE4_COLS if c not in balls.columns]
    if missing:
        raise ValueError(f"Columns missing: {missing}. Re-run wave4.py.")
    print(f"  All Wave 2+4 columns present")
else:
    print(f"\n{WAVE4_BALLS} not found — computing inline...")
    if not WAVE2_BALLS.exists():
        raise FileNotFoundError(f"Need {WAVE2_BALLS}. Run wave2.py first.")
    balls = pd.read_csv(WAVE2_BALLS)
    sys.path.insert(0, str(Path(__file__).parent))
    from wave4 import compute_rolling_player_stats, compute_over_par, attach_features
    balls["match_id"] = balls["match_id"].astype(str)
    mt = matches.copy(); mt["match_id"] = mt["match_id"].astype(str)
    md = mt[["match_id", "date"]].drop_duplicates("match_id")
    stats = compute_rolling_player_stats(balls, md)
    ms = mt.sort_values("date").reset_index(drop=True)
    tc = ms.iloc[int(len(ms) * 0.70)]["date"]
    vp = compute_over_par(balls, md, tc)
    balls = attach_features(balls, stats, vp, md)
    balls.to_csv(WAVE4_BALLS, index=False)
    print(f"  Saved to {WAVE4_BALLS}")
...
print(f"Weather merge  : {found}/{before}")
...
if "start_time" in matches.columns:
    ...
    print("INFO: defaulting to evening.")
...
def build_state_table(balls, matches):
    ...
    df["runs_so_far"] = df.groupby(["match_id", "innings"])["total_runs"] \
        .transform(lambda x: x.shift(1, fill_value=0).cumsum())
    df["wickets_lost"] = df.groupby(["match_id", "innings"])["wicket"] \
        .transform(lambda x: x.shift(1, fill_value=0).cumsum())
    df["legal_ball_before"] = df.groupby(["match_id", "innings"])["is_legal"] \
        .transform(lambda x: x.shift(1, fill_value=0).cumsum()).clip(0, 120)
    df["balls_remaining_before"] = (120 - df["legal_ball_before"]).clip(0, 120)

    df["runs_last_12"] = df.groupby(["match_id", "innings"])["total_runs"] \
        .transform(lambda x: x.rolling(12, min_periods=1).sum().shift(1, fill_value=0))
    df["wickets_last_12"] = df.groupby(["match_id", "innings"])["wicket"] \
        .transform(lambda x: x.rolling(12, min_periods=1).sum().shift(1, fill_value=0))
    ...
    df["pressure_index"] = np.where(
        ...
        * (1.0 + 0.50 * (1 - df["balls_remaining_before"] / 120)),
        np.nan)
    ...
    mc = (["match_id", "date", "is_day_match", "dew_likely", "season_phase",
           "venue_avg_total"] + WEATHER_FEATURES)
...
print(f"Win rate       : {state['y'].mean():.3f}")
...
pp = lr_toss.predict_proba(m_test[toss_feats])[:, 1]
print(f"  Brier post-toss : {brier_score_loss(m_test['y_match'], pp):.4f}")
...
val_date = ds.iloc[val_start]
test_date = ds.iloc[test_start]
print(f"\nSplit — train:<{val_date.date()}  val:{val_date.date()}–{test_date.date()}  "
...
# ════════════════════════════════════════════════════════════════════════════
# 9.  MODEL T — with Wave 4 player form + over_par
# ════════════════════════════════════════════════════════════════════════════
...
model_T_p10 = lgb.LGBMRegressor(**{**pTq, "objective": "quantile", "alpha": 0.05})
model_T_p90 = lgb.LGBMRegressor(**{**pTq, "objective": "quantile", "alpha": 0.95})
...
rp10 = np.minimum(model_T_p10.predict(val_i1[INN1_FEATURES]), val_i1["proj_total"].values)
rp90 = np.maximum(model_T_p90.predict(val_i1[INN1_FEATURES]), val_i1["proj_total"].values)
tv = val_i1["innings1_total"].values
margin = 0.0
for c in np.arange(0, 50, 0.5):
    if ((tv >= rp10 - c) & (tv <= rp90 + c)).mean() >= 0.75:
        margin = c; break
print(f"\nQuantile band margin: ±{margin:.1f}")
...
state.loc[i1m, "proj_total"] = model_T.predict(state.loc[i1m, INN1_FEATURES].fillna(0))
...
# 12.  MODEL W
# ════════════════════════════════════════════════════════════════════════════
# DROPPED (zero importance in v7): bowling_strength_remaining,
#   balls_remaining_before, wickets_last_12
# ADDED: batsman_sr_2yr, bowler_economy_2yr, momentum_index, pressure_index

WIN_FEATURES = [
    "p_post_toss", "elo_diff", "innings", "runs_so_far",
    ...
    # Wave 4
    "batsman_sr_2yr", "bowler_economy_2yr",
    "momentum_index", "pressure_index",
] + WEATHER_FEATURES
...
T_opt = minimize_scalar(lambda T: log_loss(vl["y"], 1 / (1 + np.exp(-lv / T))),
...
print("\n" + "=" * 70)
print("EVALUATION REPORT — TEST SET (v8 / Wave 4)")
print("=" * 70)
...
print(f"  v7 was                : 0.1720")
print(f"  Wave 4 ΔBrier         : {0.1720 - brier_overall:+.4f}")
...
── Benchmark ────────────────────────────────────────────────────────
  ...
  │ v8 (Wave 4)                                 │  {brier_overall:.4f}  │  IPL T20   │
  │ SOTA (deep learning)                        │ ~0.165    │  T20 intl  │
  └─────────────────────────────────────────────┴───────────┴────────────┘
  Inn-1: {inn1_b:.4f}  (v7=0.2297)    Inn-2: {inn2_b:.4f}  (v7=0.1093)
...
plot_reliability(test_i2["y"], [p_lr_test, pri2, pci2],
                 ...
                 "Reliability Inn2 (v8)", "reliability_innings2.png")
plot_reliability(te["y"], [p_raw_test, p_cal_test],
                 ["raw", best_cal],
                 "Reliability Both (v8)", "reliability_both_innings.png")
...
def plot_proj(mid, state, mT, mp10, mp90, feat, mg):
    mdf = state[(state["match_id"] == mid) & (state["innings"] == 1)].dropna(subset=feat)
    if mdf.empty: return
    mdf = mdf.copy()
    med = mT.predict(mdf[feat])
    p10 = np.minimum(mp10.predict(mdf[feat]), med) - mg
    p90 = np.maximum(mp90.predict(mdf[feat]), med) + mg
...
for mid in te["match_id"].unique()[:3]:
    ...
    plot_proj(mid, te, model_T, model_T_p10, model_T_p90, INN1_FEATURES, margin)
